...
void OBJLoader::loadObjectFromOBJ(OBJLOADER_OBJECT* ret, const char* fP) {
	...
	while(!feof(objFile)) {
		fscanf_s(objFile, "%s ", opt, 32);

		if(opt[0] == '#') {
			continue;
		}
		else if(!strcmp(opt, "mtllib")) { // nazwa pliku z materialem
			fscanf_s(objFile, "%s", mtlFilePath, 128);
		}
			...
			spcs = 0;
			cRe = 0;
			fgets(data, 128, objFile);
			for(unsigned int i = 0, k = 0; i < strlen(data); ++i) {
				if(data[i] == '/') ++spcs;
				if(spcs == 2) {
					spcs = 0;
